...
def unglitch(li , ndat):
	temp=False
	if ndat==1 or ndat==3 or ndat==5 or ndat==7 or ndat==8:temp=True
	for i in range( len(li)-2):
		if li[i] is None:li[i]=0
		if li[i+1] is None:li[i+1]=0
		if li[i+2] is None:li[i+2]=0
		if temp and li[i+1]>40: li[i+1]=li[i]
		if temp and li[i+2]>40: li[i+2]=li[i]
		delta=abs(li[i]-li[i+2])
		print( "{:2d} {:5d} {:6.1f} {:6.1f}".format( ndat, i , delta , li[i] ) )
...
# REMOVED AFTER PHIDGETS
listx[:,1]=unglitch(  list(listx[:,1])  ,1)   # 
listx[:,2]=unglitch(  list(listx[:,2])  ,2)   # 
listx[:,3]=unglitch(  list(listx[:,3])  ,3)   # 
listx[:,4]=unglitch(  list(listx[:,4])  ,4)   # 
listx[:,5]=unglitch(  list(listx[:,5])  ,5)   # 
listx[:,6]=unglitch(  list(listx[:,6])  ,6)   #
listx[:,7]=unglitch(  list(listx[:,7])  ,7)   # 
listx[:,8]=unglitch(  list(listx[:,8])  ,8)   #  problems for WEEK ?????
...
df=pd.DataFrame( listx  , columns=[i[0] for i in cursor.description] )
#print(df)
#######################################################END OF MYSQL 1
...
if len(listf)>0:
        dff=pd.DataFrame( listf, columns=[i[0] for i in cursor.description]  )
        print(dff)
...
print( '=======RAIN===listy======\n',listy)
if listy!=[]:
    dfr=pd.DataFrame( listy, columns=[i[0] for i in cursor.description]  )
    #print(dfr.iloc[0])
...
########################
#
#     OUT dewpoint ..... outer temperature
#
###########################
...
####ax.grid(True)
####fig.autofmt_xdate()
plt.plot( df.t, df.a ,'r.-' ,  label="out")
plt.plot( df.t, DEWP12 ,'r--', linewidth=2, label="" )
########plt.plot( df.t, df.b ,'r.-')
#plt.plot( df.t, df.c ,'k.-')
plt.plot( df.t, DEWP34 ,'k--', linewidth=2, label="")
...
# GREEN ======= MANY GLITCHES
#plt.plot( df.t, df.h ,'g.-',  label="out2")
...
#
###########################
#print("i... sending echo to dispdig.py",df['a'].iloc[0]  )
#from subprocess import Popen, PIPE
##Popen( "echo dispdig.py "+str( int(df['a'].iloc[0]) )+" | nc pix2 5699",  shell=True)
#print("i... sent echo to dispdig.py",int(df['a'].iloc[0] )  )
...
p = plt.axhline(y=flowavg,  linewidth=2, color='y', label='Energy flow')
...
plt.plot( df.t, df.b ,'r.-'  , label="out" ) 
#plt.plot( df.t, df.c ,'k.-')
plt.plot( df.t, df.d ,'k.-'  , label="kuchyne" )
#plt.plot( df.t, df.e ,'c.-')
plt.plot( df.t, df.f ,'c.-' , label="koupelna" )
...
if (INTERACTIVE==0):
        ...
        print("i... connectin to mysql", table )
        cnx = pymysql.connect(**config)
        cursor = cnx.cursor()
        query =("SELECT * FROM "+table+" ORDER BY t DESC LIMIT 2;")
        print("elektro Query:",query)
        cursor.execute(query)
        liste=np.array( list([i for i in cursor]) )
        dfe=pd.DataFrame( liste, columns=[i[0] for i in cursor.description]   )
        print(dfe)
